...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // 🔥 FIRE PRIORITY (block commands)
  if (fire_flag == 1) {
    Serial.println("Fire active - commands blocked");
    return;
  }

  // ===== MODE (only if no fire) =====
...
void readPLC() {
  ...
  // ===== MAIN READ (Temperature + Heartbeat) =====
  ...
  if (result == node.ku8MBSuccess) {
    ...
    success_count++;
    fail_count = 0;   // reset Modbus fail counter
    ...
    return; // STOP further reads if main read fails
  }
...
void safetyCheck() {
  ...
  // 🔥 FORCE SAFE STATE DURING FIRE
  if (fire_flag == 1 && plc_online) {
    node.writeSingleCoil(0, 0); // Light OFF
    node.writeSingleCoil(1, 0); // Fan OFF
    node.writeSingleCoil(2, 1); // Buzzer ON
    return;
  }

  // ⚙️ TIMEOUT ONLY IN MANUAL MODE
  if (mode_flag == 1 && millis() - lastCommandTime > 10000) {
