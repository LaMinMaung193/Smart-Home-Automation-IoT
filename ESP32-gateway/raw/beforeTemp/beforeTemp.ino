...
void readPLC() {
  ...
  if (result == node.ku8MBSuccess) {
    ...
    // Extract bits
    light_state  = coil_data & 0x01;        // bit 0
    fan_state    = (coil_data >> 1) & 0x01; // bit 1
    buzzer_state = (coil_data >> 2) & 0x01; // bit 2
...
void loop() {
  ...
  if (millis() - lastPublish > 3000) {
    ...
    readPLC();        // ✅ move here (VERY IMPORTANT)
    publishStatus();  // ✅ now sends fresh data
  }
}